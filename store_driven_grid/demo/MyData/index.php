...
$desc = false;
$sort = "name";
if (array_key_exists("sort(_name)", $_REQUEST)) {
	$desc = false;
	$sort = "name";
}
if (array_key_exists("sort(-name)", $_REQUEST)) {
	$desc = true;
	$sort = "name";
}
if (array_key_exists("sort(_abbreviation)", $_REQUEST)) {
	$desc = false;
	$sort = "abbreviation";
}
if (array_key_exists("sort(_abbreviation)", $_REQUEST)) {
...
$numRows = $end = count($ret);
$start = 0;

// Handle paging, if given.
if (array_key_exists("start", $_REQUEST)) {
	$ret = array_slice($ret, $start = $_REQUEST['start']);
}
if (array_key_exists("count", $_REQUEST)) {
	$ret = array_slice($ret, 0, $_REQUEST['count']);
	$end = $start + $_REQUEST['count'];
}
$end = $end - 1;

header("Content-Range", "items ".$start."-".$end."/".$numRows);
